...
from ..constants import (
    ...
    EINSTEIN_COLORS,
    EINSTEIN_DRINKS,
    EINSTEIN_NATIONALITIES,
    EINSTEIN_PETS,
    EINSTEIN_SMOKES,
)
from ..models import HouseAssignment, MoveResult
...
class EinsteinGame(PuzzleGame):
    ...
    def __init__(self, difficulty: str = "easy"):
        ...
        self.colors = EINSTEIN_COLORS
        self.nationalities = EINSTEIN_NATIONALITIES
        self.drinks = EINSTEIN_DRINKS
        self.smokes = EINSTEIN_SMOKES
        self.pets = EINSTEIN_PETS
    ...
    async def generate_puzzle(self) -> None:
        ...
        random.shuffle(shuffled_drinks)
        random.shuffle(shuffled_smokes)
    ...
    def _generate_clues(self) -> list[str]:
        ...
        for i in range(self.num_houses):
            ...
            if random.random() < 0.4 and " " not in house.smoke:
                pet = house.pet
    ...
    def is_complete(self) -> bool:
        ...
        for i in range(self.num_houses):
            for attr in EINSTEIN_ATTRIBUTES:
...
- Question: WHO OWNS THE FISH?"""
  ...
  quit                           - Exit the server"""
    ...
    def get_stats(self) -> str:
        ...
        """
        assigned = sum(
            1 for house in self.assignments for attr in EINSTEIN_ATTRIBUTES if house.get_attribute(attr) is not None
        )
