...
class TestSlitherlinkGame:
    ...
    def test_generate_simple_loop(self):
    ...
    def test_count_edges_around_cell(self):
    ...
    def test_count_edges_around_cell_none(self):
    ...
    def test_validate_move_horizontal_success(self):
        """Test setting a horizontal edge."""
        game = SlitherlinkGame("easy")
        game.generate_puzzle()

        success, message = game.validate_move("h", 1, 1, 1)
        assert success is True
        assert "horizontal" in message.lower()
        assert game.h_edges[0][0] == 1

    def test_validate_move_vertical_success(self):
        """Test setting a vertical edge."""
        game = SlitherlinkGame("easy")
        game.generate_puzzle()

        success, message = game.validate_move("v", 1, 1, 1)
        assert success is True
        assert "vertical" in message.lower()
        assert game.v_edges[0][0] == 1

    def test_validate_move_clear_edge(self):
        """Test clearing an edge."""
        game = SlitherlinkGame("easy")
        game.generate_puzzle()

        # Set then clear
        game.validate_move("h", 1, 1, 1)
        success, message = game.validate_move("h", 1, 1, 0)
        assert success is True
        assert "cleared" in message.lower()
        assert game.h_edges[0][0] == 0

    def test_validate_move_mark_x(self):
        """Test marking an edge as X (not part of loop)."""
        game = SlitherlinkGame("easy")
        game.generate_puzzle()

        success, message = game.validate_move("h", 1, 1, 2)
        assert success is True
        assert game.h_edges[0][0] == 2

    def test_validate_move_invalid_state(self):
        """Test with invalid state value."""
        game = SlitherlinkGame("easy")
        game.generate_puzzle()

        success, message = game.validate_move("h", 1, 1, 3)
        assert success is False
        assert "Invalid state" in message

    def test_validate_move_invalid_edge_type(self):
        """Test with invalid edge type."""
        game = SlitherlinkGame("easy")
        game.generate_puzzle()

        success, message = game.validate_move("x", 1, 1, 1)
        assert success is False
        assert "Invalid edge type" in message

    def test_validate_move_invalid_coordinates_horizontal(self):
        """Test with invalid coordinates for horizontal edge."""
        game = SlitherlinkGame("easy")
        game.generate_puzzle()

        success, message = game.validate_move("h", 0, 0, 1)
        assert success is False
        assert "Invalid" in message

        success, message = game.validate_move("h", 10, 10, 1)
        assert success is False
        assert "Invalid" in message

    def test_validate_move_invalid_coordinates_vertical(self):
        """Test with invalid coordinates for vertical edge."""
        game = SlitherlinkGame("easy")
        game.generate_puzzle()

        success, message = game.validate_move("v", 0, 0, 1)
        assert success is False
        assert "Invalid" in message

        success, message = game.validate_move("v", 10, 10, 1)
        assert success is False
        assert "Invalid" in message
    ...
    def test_is_complete_clue_not_satisfied(self):
    ...
    def test_is_complete_valid_loop(self):
    ...
    def test_is_complete_branch_detected(self):
    ...
    def test_render_grid_with_edges(self):
    ...
    def test_render_grid_with_x_marks(self):
    ...
    def test_solution_clues_match(self):
    ...
    def test_edge_grid_dimensions(self):
